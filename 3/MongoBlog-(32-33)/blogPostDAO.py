...
class BlogPostDAO:
    ...
    def add_comment(self, permalink, name, email, body):
        ...
        try:
            ...
            print 'post',post
            if post is not None:
                current_comment = post['comments']
            print 'current_comment before update',current_comment
            current_comment.append(comment)
            print 'current_comment after update',current_comment

            self.posts.update({'permalink': permalink},{"$set":{"comments":current_comment}})
